...
@implementation NSObject (MOPWRuntimeUtilities)
...
+ (IMP)_MOPW_replaceSelector:(SEL)replaceSel withMethodForSelector:(SEL)newSel isFactory:(BOOL)factoryFlag {
    ...
    // Sanity check
    MOAssert((origMethod != NULL), @"%@ - replaceSel must be implemented in order to replace it, but class %@ does not implement selector %s", (factoryFlag ? @"MOPW_replaceFactorySelector:withMethodForSelector:" : @"MOPW_replaceInstanceSelector:withMethodForSelector:"), self, replaceSel);
    MOAssert((newMethod != NULL), @"%@ - newSel must be implemented in order to replace another selector with it, but class %@ does not implement selector %s", (factoryFlag ? @"MOPW_replaceFactorySelector:withMethodForSelector:" : @"MOPW_replaceInstanceSelector:withMethodForSelector:"), self, newSel);
...
+ (IMP)MOPW_replaceInstanceSelector:(SEL)replaceSel withMethodForSelector:(SEL)newSel {
    return [self _MOPW_replaceSelector:replaceSel withMethodForSelector:newSel isFactory:NO];
}

+ (IMP)MOPW_replaceFactorySelector:(SEL)replaceSel withMethodForSelector:(SEL)newSel {
    return [self _MOPW_replaceSelector:replaceSel withMethodForSelector:newSel isFactory:YES];
}

@end

@implementation NSBundle (MOPWRuntimeUtilities)
...
@implementation Protocol (MOPWRuntimeUtilities)
...
NSString *MOPWFullMethodName(id self, SEL _cmd) {
...
NSArray *MOPWAllClassesImplementingInstanceSelector(SEL selector) {
...
NSArray *MOPWAllClassesImplementingFactorySelector(SEL selector) {
    return _MOAllClassesSatisfyingTest(_MOClassFactoryImplements, (void *)selector, NULL);
}

BOOL MOPWKitAllowsMethodReplacement() {
    ...
    if (!registered) {
        NSDictionary *dict = [[NSDictionary allocWithZone:NULL] initWithObjectsAndKeys:@"YES", @"MOPWKitAllowsMethodReplacement", nil];
        [ud registerDefaults:dict];
        [dict release];
    }
    BOOL result = [ud boolForKey:@"MOPWKitAllowsMethodReplacement"];
    [pool release];
    
    return result;
}

void MOPWKitSetAllowsMethodReplacement(BOOL flag) {
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];

    [ud setObject:(flag ? @"YES" : @"NO") forKey:@"MOPWKitAllowsMethodReplacement"];
}
